...
export default async function JewelryPage() {
  const listings = await prisma.listing.findMany({
    where: { status: "APPROVED", category: "JEWELRY" },
  ...
  return (
    <main className="min-h-screen bg-background px-8 py-12">
      <div className="max-w-6xl mx-auto">
        <h1 className="text-3xl font-bold text-foreground mb-2">Jewelry</h1>
        <p className="text-secondary mb-10">
          Exquisite luxury jewelry pieces available for purchase.
        </p>

        {listings.length === 0 ? (
          <p className="text-secondary">No jewelry available yet.</p>
        ) : (
